...
from tools_db.tool_handler import ToolDBHandler
...
class DataBaseManager():
    ...
    def user_get_tool(self, tool_no):
        ...
        data.append(f"T{tool.number}")
        
        pocket_exist = self.session.query(exists().where(Pockets.tools_id == tool_no)).scalar()
        if pocket_exist:
            pocket = self.session.query(Pockets).filter(Pockets.tools_id == tool_no).one()
            data.append(f"P{pocket.slot_pos}")
        else:
            data.append(f"P0")
            
        offsets = self.session.query(Offsets).filter(Offsets.tools_id == tool_no).one()
        
        data.append(f"D{offsets.diameter}")
        data.append(f"X{offsets.x_offset}")
        data.append(f"Y{offsets.y_offset}")
        data.append(f"Z{offsets.z_offset}")
        data.append(f"A{offsets.a_offset}")
        data.append(f"B{offsets.b_offset}")
        data.append(f"C{offsets.c_offset}")
        data.append(f"U{offsets.u_offset}")
        data.append(f"V{offsets.v_offset}")
        data.append(f"W{offsets.w_offset}")
...
def main():
    ...
    if not os.path.exists("tools.sqlite"):
        init_db()
