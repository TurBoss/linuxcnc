...
import array, time, atexit, tempfile, shutil, errno, _thread, select, re, getopt
import traceback
...
try:
    print(inifile.find("TRAJ","COORDINATES").lower().replace(b" ",b""))
    trajcoordinates=inifile.find("TRAJ", "COORDINATES").lower().replace(b" ",b"")
except:
...
for a in range(linuxcnc.MAX_AXIS):
    ...
    letter = b"xyzabcuvw"[a]
    if not (letter in trajcoordinates): continue
    if letter in b"abc":
        axis_type[a] = "ANGULAR"
    else:
        axis_type[a] = "LINEAR"
    section = "AXIS_%s" % str(letter).upper()
